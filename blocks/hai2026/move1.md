...
It needs to reach the **copper block** ![Copper Block](https://raw.githubusercontent.com/ReWrite-Media/makecode/master/blocks/hai2026/img/copper_block.png "Copper Block")
...
#### ~ tutorialhint
Find the emerald block and the copper block in the maze. Each floor square is one move.
...
## Step 7
Almost there! Go **forward** until you are in the same row as the copper block. Add a ``||agent: agent move forward||`` block and count the squares.

#### ~ tutorialhint
Add a ``||agent: agent move forward||`` block. Count the squares until the Agent lines up with the copper block.

## Step 8
Last move! Go **left** to the copper block. Add a ``||agent: agent move||`` block, pick **left**, and count the squares.

#### ~ tutorialhint
Add a ``||agent: agent move||`` block and change it to **left**. Count the squares between the Agent and the copper block.

## Step 9
Type **run** in chat and watch the Agent go! Did it reach the copper block? If it got stuck, check your numbers and try again. You can do it!
